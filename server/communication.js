module.exports = function(server, utils) {
  if (!server.socket) {
  ...
  server.socket.on('connection', function(client) {
    ...
    **/
    client.on('client:init', function(data) {
      ...
      // Platform
      data.platform = utils.getPlatformInfo(data.userAgent);

      // Cache params
      for (var i = 0; i < data.style.length; i++) {
        if (data.style[i].type === -2) {
          utils.removeParams(data.style[i]);
        }
      }

      map[session] = map[session] || {};
      map[session][id] = map[session][id] || {};
      utils.normalizeNodes(data.style, map[session][id], function(style) {
              // hash: hash,
    ...
    client.on('change:response', function(data) {
      if (data.change) {
        ...
        if (data.enabled) {
          ...
          utils.updateNode(c.data.style, data.hash, data.data.name, data.newValue, true);
          map[data.session][data.device][data.hash].enabled = true;
        } else {
          utils.updateNode(c.data.style, data.hash, data.data.name, data.newValue, false);
